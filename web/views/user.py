...
def search_text():
    ...
    return jsonify(texts=["%s (%s, <span id=\"loc\">%s</span>)" % (i.text, time.asctime(time.localtime(i.time)), i.location) for i in list(Text.select().where(Text.user == u, Text.text % ("%%%s%%" % query)))])
